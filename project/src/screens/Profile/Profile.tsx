...
import { User, Mail, ShieldCheck, Key, Settings, Sparkles } from "lucide-react";
import { Button } from "../../components/ui/button";

export const Profile = () => {

  useEffect(() => {
    // Ideally fetch from an API like /api/auth/me
    // Since we didn't add the /me route in authController, we can just mock or grab from localStorage if we had it.
    // We will show a placeholder UI that matches the theme.
  }, []);
        ...
        <div className="bg-[#ffffff05] border border-[#ffffff10] rounded-3xl p-8 backdrop-blur-md shadow-2xl flex items-center gap-8">
           ...
           <div className="flex-1">
              <h2 className="text-3xl font-bold text-white mb-2 flex items-center gap-2">
                 Alex Rivera <ShieldCheck className="text-[#a855f7] w-6 h-6" />
              </h2>
              <div className="flex flex-col gap-2 mt-4 text-slate-400">
                <span className="flex items-center gap-2"><Mail className="w-4 h-4" /> alex.vault@sanctuary.io</span>
                <span className="flex items-center gap-2"><Sparkles className="w-4 h-4" /> Member since Dec 2023</span>
              </div>
           </div>
           ...
           <div className="bg-[#ffffff05] border border-[#ffffff10] rounded-3xl p-8 backdrop-blur-md">
             ...
             <div className="space-y-4">
                <div className="space-y-2">
                   ...
                   <div className="bg-[#0f172a80] rounded-xl px-4 py-3 border border-[#33415580] text-white">Alex Rivera</div>
                </div>
                <div className="space-y-2">
                   <label className="text-xs text-slate-500 uppercase tracking-widest">Email Address</label>
                   <div className="bg-[#0f172a80] rounded-xl px-4 py-3 border border-[#33415580] text-white">alex.vault@sanctuary.io</div>
                </div>
                <Button className="w-full mt-4 bg-transparent border border-[#7919e6] text-[#7919e6] hover:bg-[#7919e6] hover:text-white transition-colors">
                  Update Identity
                </Button>
             </div>
           </div>
